# r_notebook baseline checks

Baseline checks to run against a newly built `r_notebook` image, simulating
normal user activity to catch regressions before the image ships.

Run everything with:

```bash
bash tests/r_notebook/run_baseline_checks.sh
```
...
- **`baseline_check.R`** — the same install / use / clean up / validate
  pattern, but for R: `install.packages()` a CRAN package, use it for a
  calculation, `remove.packages()` it, then validate the removal actually
  took effect.
