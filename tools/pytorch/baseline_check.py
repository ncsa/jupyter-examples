...
def main():
    ...
    # has pandas/tabulate cached in sys.modules from the import above). Note
    # this checks re-importability of whatever was actually newly installed
    # (package_names), not a fixed package list -- on base images that
    # already ship one of PACKAGES_TO_INSTALL (e.g. pytorch's scipy-notebook
    # base ships pandas), that package is correctly never uninstalled, so it
    # should stay importable rather than being flagged as a cleanup failure.
    ...
    for name, importable in still_importable.items():
        if importable:
            failures.append(f"{name} still importable in a fresh process after uninstall")
